...
it("returns slices correctly when in bounds of the buffer", () => {
    ...
    const r1 = toJsArray(a.readUntil(s => s.size() === 2));
    const r2 = toJsArray(a.readUntil(s => s.size() === 2));
    const r3 = toJsArray(a.readUntil(s => s.size() === 3));
    const r4 = toJsArray(a.readUntil(s => s.size() === 1));
...
it("returns the rest of the buffer when going out of bounds", () => {
    ...
    const a = $(...arr);
    expect(toJsArray(a.readUntil(() => false))).toEqual(arr);
});